...
class App:
            ...
            for animName in self.navTree[entityName].keys():
                ...
                for spriteFilename in self.navTree[entityName][animName].keys():
                    ...
                    for pointName in self.navTree[entityName][animName][spriteFilename].keys():
                        ...
                        if self.navTree[entityName][animName][spriteFilename][pointName]["posX"] == None:
                            posText = "Not Set"
                        else:
                            posX: float = self.navTree[entityName][animName][spriteFilename][pointName]["posX"]
                            posY: float = self.navTree[entityName][animName][spriteFilename][pointName]["posY"]
                            posText = f"({posX}, {posY})"
                        ...
                        self.PE_itemDict[entityName][animName][spriteFilename][pointName] = {
                            "frame": itemFrame,
                            "nameLabel": nameLabel,
                            "posLabel": posLabel,
    ...
    def SelectEntity(self, entity: str) -> None:
        ...
        self.ClearItemsFromPointEditor()

        self.ClearButtonsFromSpriteSelector()

        self.ClearButtonsFromAnimationSelector()

        self.AddButtonsToAnimationSelector()
    ...
    def SelectAnimation(self, anim: str) -> None:
        ...
        self.AddButtonsToSpriteSelector()
    ...
    def SelectSprite(self, sprFilename: str, index: int) -> None:
        ...
        self.AddItemsToPointEditor(self.currEntity, self.currAnim, self.currSpr)
    ...
    def AddButtonsToAnimationSelector(self) -> None:
    ...
    def ClearButtonsFromAnimationSelector(self) -> None:
    ...
    def AddButtonsToSpriteSelector(self) -> None:
        # add the new buttons to the list
    ...
    def ClearButtonsFromSpriteSelector(self) -> None:
    ...
    def AddItemsToPointEditor(self, entityName: str, animName: str, sprFilename: str) -> None:
    ...
    def ClearItemsFromPointEditor(self) -> None:
    ...
    def OpenPopupAddPoint(self) -> None:
        popup: tk.Toplevel = tk.Toplevel(self.root)
        pointName: str = None
        descLabel: ttk.Label = ttk.Label(popup, text="Please enter the name of the new point in the text box below, and then click the \"confirm\" button.")
        descLabel.grid(column=0, row=0)
        textBox: ttk.Entry = ttk.Entry(popup, textvariable=pointName)
        textBox.grid(column=0, row=1)
        confirmButton: ttk.Button = ttk.Button(popup, text="Confirm", command=partial(self.ClosePopupAddPoint, popup, pointName))
        confirmButton.grid(column=0, row=2)

    def ClosePopupAddPoint(self, popup: tk.Toplevel, pointName: str) -> None:
        assert pointName is not None # make sure they actually put something in for the name
        assert len(pointName) > 0 # make sure the name is not an empty string
        assert pointName.isalnum() == True # make sure the name of the point is alphanumeric
        # TODO: add the point to the point data
        popup.destroy()
    ...
    def AddNewPoint(self, pointName: str) -> None:
        for animName in self.navTree[self.currEntity].keys():
            for sprFilename in self.navTree[self.currEntity].keys():
                pass
    ...
    def ValidateData(self) -> None:
        # for each entity, make sure that the same point names exist for every sprite in every animation
        assert False
    ...
    def PopulateNavTree(self) -> None:
        ...
        for entityName in subdirectories:
            ...
            files: list[str] = jns.GetFilesWithConvention(currDirectory, jns.CONVENTION_ENTITY)
